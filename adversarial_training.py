...
def adversarial_training(train_start=0, train_end=60000, test_start=0,
  ...
  adv_acc_metric = get_adversarial_acc_metric(model, fgsm, fgsm_params)#???adv_acc_metric return keras.metric.categorical_crossentropy
  model.compile(
  ...
  _,acc, adv_acc = model.evaluate(x_test, y_test,
                                   batch_size=batch_size,
                                   verbose=1)#为什么model.evaluate会返回对抗样本准确率,返回的第一个值是啥
...
def get_adversarial_acc_metric(model, fgsm, fgsm_params):
  def adv_acc(y, _):
    ...
    preds_adv = model(x_adv)#x_adv为生成的对抗样本标签
...
def main(argv=None):
  ...
  adversarial_training(nb_epochs=FLAGS.nb_epochs,
